                ...
                <form action="{{ route('quiz.play', $quiz) }}" method="POST">
                    ...
                    <div class="mb-6">
                        <label class="block text-sm font-bold text-gray-700 mb-1">
                            Tét (Pont):
                            <span class="text-indigo-600" x-text="bet + ' PT'"></span>
                        </label>

                        {{-- Normál Tét Csúszka/Input --}}
                    ...
                    <div class="mb-6" x-show="mode === 'odds'">
                        <label class="block text-sm font-bold text-gray-700 mb-2">Kérdések száma:</label>
                        <div class="grid grid-cols-3 gap-4">
                            <template x-for="count in [10, 20, 30]">
                                <button type="button"
                                        @click="questionCount = count"
                                        :class="questionCount === count ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-700'"
                                        class="py-2 rounded-xl font-bold transition"
                                        x-text="count + ' kérdés'">
                                </button>
                            </template>
                        </div>
                    ...
                    {{-- 4. IDŐKORLÁT (Módosító szorzó) --}}
                    <div class="mb-6">
                        <label class="block text-sm font-bold text-gray-700 mb-2">Válaszadási idő (Kérdésenként):</label>
                        <div class="grid grid-cols-3 gap-4">
                            <button type="button" @click="time = 60" :class="time === 60 ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-gray-200 bg-white text-gray-600'" class="p-3 border-2 rounded-xl text-center font-bold">
                                60 sec <span class="block text-xs font-normal text-gray-500">(Alap szorzó: 1.0x)</span>
                            </button>
                            <button type="button" @click="time = 30" :class="time === 30 ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-gray-200 bg-white text-gray-600'" class="p-3 border-2 rounded-xl text-center font-bold">
                                30 sec <span class="block text-xs font-normal text-indigo-600">(+1.5x Módosító)</span>
                            </button>
                            <button type="button" @click="time = 10" :class="time === 10 ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-gray-200 bg-white text-gray-600'" class="p-3 border-2 rounded-xl text-center font-bold">
